...
const find_volume_planifie = async (req, res) => {
    try {
      ...
      const planifier = await Promise.all(users.map(async countObject => {
...
const find_volume_prepare = async (req, res) => {
    try {
      ...
      const prepared = await Promise.all(users.map(async countObject => {
        const util = countObject.toJSON()
        ...
        const uniqueId = [];
        const folio_non_preparedfilter = folio_non_prepared.filter((element) => {
...
const agent_desarchivage_preparation = async (req, res) => {
  try {
    const { startDate, endDate } = req.query
    // Date filter
    var dateWhere = {}
